...
{组件} 存在 {漏洞类型}，漏洞源于 {函数/接口/配置} 存在 {具体问题}。
{攻击者身份} 可通过 {攻击向量} {具体攻击方式}，导致 {具体危害}。
攻击者可利用此漏洞实现 {完整攻击场景}。

## 漏洞概述（外部）

{对外披露版，2-3 句。使用标准漏洞描述语言，简洁客观，不暴露内部实现细节。}
...
- 目标系统：{Kylin V11 / openKylin / 其他}
- 测试方式：{虚拟机 / 物理机 / Agent on Target}
- 测试用户：{普通用户 / 无特殊权限}

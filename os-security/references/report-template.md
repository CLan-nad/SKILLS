...
**漏洞描述的第一句必须是加粗陈述句**：`<组件> 存在 <漏洞类型> 漏洞` —— 先给结论，再展开。
例：`kylin-video 组件存在不可信媒体文件名 SQL 注入漏洞`。
...
> 章节序：标题 → 目标组件描述 → 漏洞描述(首句陈述 + 根因 + 对外脱敏) → 漏洞类型(CWE) → 危害等级 → CVSS 3.1 评分 → 漏洞利用的可能性 → 漏洞触发的前提条件 → 实际危害分析 → 漏洞原理 → 漏洞详情 → 验证情况(六阶段) → PoC 验证 → 修复建议 → 进一步分析建议。
...
{组件名 / 版本 / 来源包 / 安装路径 / 运行形态（守护进程·GUI 会话应用·SUID 二进制·内核模块）/ 运行身份（root·登录用户）}
...
> **强制**：本节首句必须是如上**加粗陈述句**（`<组件> 存在 <漏洞类型> 漏洞`），先结论后展开。

**对外脱敏描述**：{2–3 句标准漏洞语言；不出现内部函数名 / 字段偏移 / 私有接口名。}
...
{严重(9.0+) / 高危(7.0-8.9) / 中危(4.0-6.9) / 低危(0.1-3.9)}

## CVSS 3.1 评分

- 向量：`CVSS:3.1/AV:{X}/AC:{X}/PR:{X}/UI:{X}/S:{X}/C:{X}/I:{X}/A:{X}`
- 评分：{0.0-10.0}
- **维度依据**（逐项一句话）：
  - **AV**：{为什么取该值}
  - **AC**：{...}  - **PR**：{...}  - **UI**：{...}  - **S**：{...}
...
- **权限影响**：{跨权限域（可越权） / 同权限域（不可越权）}（扩展模式必填）
  > 评分规则、常见错误、危害判定对照见本文件「CVSS 3.1 严格评分指南」。

## 漏洞利用的可能性

{触发难度 / 是否需交互 / 是否需非默认配置 / 可重复性与稳定性}

## 漏洞触发的前提条件

| 条件 | 是否满足 | 依据 |
|------|---------|------|
| {进程运行 / 门控开启 / 字段可控 / 总线可访问 …} | {是/否} | {证据} |

## 实际危害分析

| 行为 | 利用的能力或工具 | 验证状态 |
|------|----------------|----------|
| {如：写入任意文件} | {如：cap_dac_override + /bin/sh} | ✅ 已验证 |
| {如：读取 /etc/shadow} | — | ❌ 未验证 |
...
> 六阶段；每阶段：`命令` → 关键回显 → 结论（一律用系统级证据，不依赖接口返回值）。
...
{`vuln-00N/poc.py` 实机运行输出；末行判定必须是 `漏洞存在` / `漏洞不存在` / `poc 执行失败，请调整环境或改用其他方式验证`}
...
> **字段映射（旧版 → 新版）**：`漏洞信息` → `目标组件描述` + `漏洞详情`（等级/CWE/评分拆到各自小节）；`漏洞概述（内部）` → `漏洞描述`；`漏洞概述（外部）` → `漏洞描述 · 对外脱敏描述`；`漏洞原因` → `漏洞原理`；`复现步骤 / POC 脚本` → `验证情况` + `PoC 验证`。下方「填写示例 1–5」沿用旧字段，对照本表阅读。

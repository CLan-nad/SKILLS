...
**未授权调用时即注入**：按命名推测功能、以普通用户调用危险方法时，**字符串实参直接用注入载荷**——一次调用同时回答"是否未授权"与"是否注入"，通常几分钟即定论（逆向只用来解释结果，不用来下判定；静态 ≤3 命令，禁全面反汇编/函数边界/变量追踪，先枚举输入形状）。**返回 AccessDenied 则注入无意义**（先解决鉴权，见本节末）。此步先于任何静态分析：
...
判定手段：**黑盒探测优先**（调用即注入）；静态仅用于解释（`objdump` 找 `lea "bash"` + `"-c"` 组合、`nm -D` 查 `system`/`popen` 导入；≤3 命令，禁函数边界/变量追踪）。**禁止以一个汇点的结论外推另一个汇点**。

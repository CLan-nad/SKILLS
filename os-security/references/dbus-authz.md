...
4. **配置门控维度**：触发路径常被**默认关闭**的配置开关门控（如 `General/clearlist_when_exit`）。**不预设先后顺序，按实测验证**——"先开闸再注入"与"先注入再开闸"都可能成立，以实测行数/报错为准；若入口确因门控拒写，再调整顺序并**记录实测证据**（行数、报错）。门控溯源（键字符串 → 全局偏移 → 判定分支）见 SKILL.md「可达性回溯」。

...
| 系统调用 | SUID/Cap 二进制 | `strings`/`strace`（静态只定位，输入形状枚举先行）| 危险函数 → 参数注入 → 权限绕过 |
...
> 本文件是从"入口→攻击面"枚举；若已知疑似 sink 需反查入口，见 SKILL.md「可达性回溯：疑似 sink → 攻击者入口」（只定位；输入形状枚举先行）。
...
- **开源一致（SKILL.md「判定」拒绝标准第 5 条）验证程序**：比对包版本与上游 tag → 抽查关键文件 md5/diff → 查发行版补丁目录；**确认一致才跳过**，厂商打过补丁的按自研对待。

**执行组件**：**不运行就测试等于白测。** 有 `.service` → `systemctl start`；有 `.ko` → `insmod`/`modprobe`；已在运行的检查状态即可。无法启动 → 记录原因、跳过运行时测试只测静态项，并按「目标不可运行」结论收尾。

### 门禁检查

每个攻击面先确认适用性，不适用则跳过（结论写一行原因）：
...
| 非特权目标 | 权限摸底显示全部攻击面均在登录用户权限域内 | 触发扩展模式门控（见 SKILL.md「判定」） |

...
对可取字符串参数的 SUID/Cap 二进制，**调用时直接用注入载荷当 argv**——一次执行同时测"能否调用"与"是否注入"（带着载荷去探测，详见 dbus-authz.md 同名节；五形载荷与引号选形同彼处）：
...
> **仅 SKILL.md「主流程」第 4 步情形 (b)（疑似高危入口利用失败）启用本节**；情形 (a) 只做轻量根因定位，不得展开。

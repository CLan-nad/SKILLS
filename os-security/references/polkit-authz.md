...
> 判定"Polkit 被绕过/未授权"**之前**，先确认 Polkit 在该路径上**确实生效**——否则"无密码调用成功"可能只是"服务根本没调 Polkit"。完整判定表（机制/生效性证据/未加载 vs 绕过）见 SKILL.md「安全机制生效性核验」；要点：
> - `pkaction --action-id <action> --verbose` 确认 action 存在且本应拦截；
> - `strings <daemon> | grep -iE 'PolicyKit|CheckAuthorization'` 无命中 = **未集成**（配置缺陷，不是绕过）；
> - `pkcheck --action-id <action> --process $$`（rc=2 = 本应拦截，非交互基线）。
